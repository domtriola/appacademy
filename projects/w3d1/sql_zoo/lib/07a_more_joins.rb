...
def expensive_tastes
  ...
  execute(<<-SQL)
    SELECT
      styles.style,
      (SUM(albums_tracks.price) / SUM(albums_tracks.count))
    FROM
      styles
    JOIN ( -- tracks / album
      SELECT
      ...
      GROUP BY
        albums.asin
    ) as albums_tracks ON styles.album = albums_tracks.asin
    GROUP BY
      styles.style
    ORDER BY
      (SUM(albums_tracks.price) / SUM(albums_tracks.count)) DESC
    LIMIT
      5
  SQL
end
