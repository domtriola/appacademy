...
def vanity_projects
  ...
  Movie.select("movies.title, movies.id, actors.name").joins(:actors)
...
def most_supportive
  ...
  # Actor.select("actors.id, actors.name, COUNT(*) AS count").joins(:castings)
  # .group("actors.id").having("castings.ord > 1")
